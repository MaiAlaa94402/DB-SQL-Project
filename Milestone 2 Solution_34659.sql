...
);insert into Assignment (cid,number,type,content) values 
...
sid int REFERENCES Student(id) on DELETE CASCADE on UPDATE CASCADE,
cid int REFERENCES Course(id) on DELETE NO ACTION on UPDATE NO ACTION,
instId int REFERENCES Instructor(id) on DELETE NO ACTION on UPDATE NO ACTION,
payedfor BIT,
grade decimal(5,2),
...
insert into StudentTakeCourse (sid,cid,insid) values (1,4,1)
select * from Student
...
BEGIN
IF (EXISTS(SELECT * FROM Users WHERE id=@Sid))
SELECT A.content FROM Assignment A INNER JOIN Course C 
ON A.cid = C.id WHERE C.id = @courseId
ELSE
print 'not a user'
END
go
exec viewCertificate 4,1
-select * from Assignment
...
--select * from Course
--select * from StudentTakeAssignment
--exec viewAssignGrades @assignnumber=1,@assignType='quiz',@cid=4,@sid=1,@assignGrade output
...
END
exec viewCertificate @cid=4,@sid=1
select * from StudentCertifyCourse
--insert into StudentCertifyCourse values (1,4,'4/4/2020')
